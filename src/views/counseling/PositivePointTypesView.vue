<template>
  <div class="space-y-6">
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <h1 class="text-2xl font-bold text-gray-800">Jenis Catatan Positif</h1>
    ...
    <BaseModal v-model="showModal" :title="isEditing ? 'Edit Jenis Catatan Positif' : 'Tambah Jenis Catatan Positif'" maxWidth="lg">
      <form @submit.prevent="saveData" class="space-y-4">
        <BaseInput id="name" v-model="form.name" label="Nama Catatan Positif" placeholder="Contoh: Membantu Teman, Aktif Pramuka" required />
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Kategori <span class="text-red-500">*</span></label>
            <select v-model="form.category" class="block w-full rounded-md border-gray-300 px-3 py-2 border bg-white/50" required>
              <option value="" disabled>Pilih Kategori</option>
              <option value="sosial">Sosial / Kemanusiaan</option>
              <option value="organisasi">Organisasi / Kepemimpinan</option>
              <option value="ibadah">Keagamaan / Ibadah</option>
              <option value="akademik">Akademik / Kedisiplinan</option>
              <option value="lainnya">Lainnya</option>
            </select>
          </div>
          <BaseInput id="points" v-model="form.points" type="number" label="Poin (Default)" placeholder="Contoh: 10" required />
        </div>
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
          <textarea v-model="form.description" rows="3" class="block w-full rounded-md border-gray-300 focus:border-primary focus:ring-primary sm:text-sm px-3 py-2 border bg-white/50" placeholder="Deskripsi poin positif (opsional)"></textarea>
        </div>
      </form>
      <template #footer>
    ...
    <BaseModal v-model="showDeleteModal" title="Konfirmasi Hapus" maxWidth="sm">
      <p class="text-sm text-gray-500">Yakin ingin menghapus jenis catatan positif <b>{{ currentItem?.name }}</b>?</p>
      <template #footer>
...
import { positivePointTypeService } from '@/services/api/positivePoint.service'
import { useToast } from '@/composables/useToast'

const { success, error: showError } = useToast()
const items = ref([]); const loading = ref(false); const total = ref(0); const currentPage = ref(1); const limit = ref(10); const search = ref(''); const sortBy = ref('name'); const sortDesc = ref(false)
...
const defaultForm = { name: '', category: '', points: 0, description: '' }
const form = reactive({ ...defaultForm })

const columns = [
  { key: 'name', label: 'Nama Catatan Positif', sortable: true },
...
const fetchData = async () => { loading.value = true; try { const r = await positivePointTypeService.getAll({ page: currentPage.value, limit: limit.value, search: search.value, sortBy: sortBy.value, sortDesc: sortDesc.value }); if (r.success) { items.value = r.data.positivePointTypes; total.value = r.data.totalItems; } } catch (e) { showError('Gagal mengambil data') } finally { loading.value = false } }
...
const saveData = async () => { 
  if (!form.name || !form.category || form.points === null || form.points === undefined) return showError('Lengkapi form dengan benar!'); 
  saving.value = true; 
  try { 
    if (isEditing.value) await positivePointTypeService.update(currentItem.value.id, form); 
    else await positivePointTypeService.create(form); 
...
const deleteData = async () => { 
  deleting.value = true; 
  try { 
    await positivePointTypeService.delete(currentItem.value.id); 
