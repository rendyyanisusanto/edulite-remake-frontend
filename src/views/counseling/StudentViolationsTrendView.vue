<template>
  <div class="space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        <div class="flex items-center gap-2">
            ...
            <h1 class="text-2xl font-bold text-gray-800">Tren Pelanggaran</h1>
        </div>
        <p class="text-sm text-gray-500 mt-1 ml-7">Statistik perubahan jumlah pelanggaran berdasarkan waktu.</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4">
        <div class="md:col-span-1">
          ...
          <select v-model="filters.period" @change="fetchData" class="block w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500">
    ...
    <!-- Chart Section -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 h-[500px] flex flex-col">
...
const fetchData = async () => {
  loading.value = true
  try {
    const r = await studentViolationService.getTrendStatistics({ 
    ...
    if (r.success) { 
        rawTrends.value = r.data || []
    }
  } catch (e) { 
      showError('Gagal mengambil data tren pelanggaran') 
  } finally { 
      loading.value = false 
  }
}

// Format period label for better readability
...
const chartData = computed(() => {
    const labels = rawTrends.value.map(t => formatPeriodLabel(t.period_label))
    const data = rawTrends.value.map(t => parseInt(t.total) || 0)
